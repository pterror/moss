...
**Draft upstream issue for `bearcove/arborium`** (not filed — no repo-write credentials from this environment; a human should paste this, checking `github.com/bearcove/arborium/issues` first for an existing report):

> **Title: `arborium-zsh` ships `"externals": []` and no `scanner.c` — upstream tree-sitter-zsh has a ~92KB hand-written scanner that's missing from the packaged crate**
>
> `arborium-zsh` (reproduced on both published versions, 2.17.0 and 2.18.1 — their `grammar/src/grammar.json`, `parser.c`, `node-types.json`, `queries/`, and `samples/` are byte-identical, only `build.rs`/`Cargo.toml`/build hash differ) packages a grammar whose `grammar.json` declares `"externals": []` and ships no `scanner.c` anywhere in the crate.
>
> The crate's own README records its upstream provenance as `georgeharker/tree-sitter-zsh` commit `c50ded6979eb7aae8b8da5da9ac2e9e984804881`. At that exact commit, upstream's `grammar.js` declares ~40 external tokens (`variable_name`, `heredoc_start`/`_content`/`_end`, `test_operator`, `regex`, brace/dollar disambiguation tokens, etc.) and upstream's `src/scanner.c` is a 91,929-byte hand-written file implementing them — the same class of scanner-driven reserved-word/argument disambiguation that `tree-sitter-bash` relies on for correct parsing.
>
> Without that scanner, the packaged `arborium-zsh` grammar cannot disambiguate keywords (`if`/`for`/`while`/`case`) from plain words or delimit command arguments. Reproduced empirically: a compiled `zsh.so` built fresh from `arborium-zsh` fails to parse even `echo hi` as a single `command` node with an argument (it splits into an ERROR-wrapped first word and an unrelated second `command`), and `if`/`for`/`while`/`case`-led statements never produce their respective statement nodes from any realistic input tried (with/without brackets, `[ ]` vs `[[ ]]`, single/multi-line, nested/top-level).
>
> This looks like a packaging-pipeline defect (`cargo xtask gen --name zsh` per the repo's own README dropping the external scanner during generation for this specific grammar) rather than a defect in upstream `tree-sitter-zsh` itself — the upstream repo has the scanner; the packaged crate doesn't.
>
> **Repro:** build `arborium-zsh` (any version), parse `echo hi\nif [[ 1 -eq 1 ]]; then echo yes; fi\n` with the resulting grammar, observe the tree is mostly bare `word`/`file_descriptor` tokens plus several `ERROR` nodes, with no `command` or `if_statement` node anywhere.
>
> **Expected:** the packaged crate includes upstream's `scanner.c` (or an equivalent compiled-in external scanner), matching how other grammars' packaged externals work.
