...
- **Orphaned `git stash` triage (2026-07-28): all 3 entries classified as superseded/obsolete — PENDING USER DECISION to drop, none dropped.** A merge agent earlier the same day surfaced `stash@{0}`/`stash@{1}` (both `"WIP on master: 5ba109a8 feat(facts): persistent query cache"`) without investigating content; this session found a third, `stash@{2}` (`"WIP on worktree-agent-ab50a979: 97265e1e ..."`), and inspected all three via `git stash show -p`:
  - `stash@{0}` (touches `commands/view/mod.rs`, `output.rs`, `service/view.rs`, 71 lines): partial WIP for a `normalize view chunk` command (chunked/around-pattern large-file navigation). The stash message's commit reference (`5ba109a8`, "persistent query cache") only names what HEAD was at stash time — unrelated to the stash's actual content, which is a separate feature. **Superseded**: `view chunk` shipped in full as `3e1e5847` ("feat(view): --chunk N and --around pattern for large file navigation"), a direct descendant of the same `5ba109a8` parent — `crates/normalize/src/commands/view/chunked.rs` is tracked on master today with equivalent functionality. (Note: the stash predates `-u`/untracked-file inclusion, so the new `chunked.rs` file itself was never captured in the stash at all — only the tracked-file diffs registering it were; the untracked file itself was presumably lost or superseded on disk before the eventual real implementation landed.)
  - `stash@{1}` (touches `commands/sessions/mod.rs`, `commands/view/mod.rs`, `output.rs`, `service/sessions.rs`, `service/view.rs`, 311 lines): superset of `stash@{0}` — same `view chunk` WIP plus `sessions parallelization`/`sessions heatmap`/`sessions cost` subcommands. **Superseded**: all three sessions subcommands shipped as `f66961c5` ("feat(sessions): parallelization hints, file edit heatmap, cost breakdown"), also a direct child of `5ba109a8`; `crates/normalize-sessions/src/{parallelization,heatmap,cost}.rs` exist on master today (the sessions command surface was later extracted into its own crate via `2ca71235`).
  - `stash@{2}` (`Cargo.lock` lockfile drift + `crates/normalize/src/service/rules.rs`, 147 lines): WIP for a `rules config-show` subcommand (effective rules config: global-allow patterns, per-rule overrides, SARIF tools). **Superseded**: an identically-scoped `rules config-show` shipped as `44617616` (tagged `v0.2.0`, "feat(rules): add config-show command to display effective rules configuration") and was later folded into the unified `config show --section rules` (`25cb24ab`, "feat(config): show per-rule overrides in config show --section rules") — `crates/normalize/src/service/config.rs`'s `show()` method covers this today; the standalone `RulesService` itself has since moved to `crates/normalize-rules/src/service.rs` and has no `config_show` method (rendered redundant by the unified command).
  - **Recommendation: safe to `git stash drop` all three** — every feature each one contains is independently verified present and shipped on master under (in some cases renamed/relocated) equivalent commands. Not dropped per this session's explicit instruction to report, not act, on stash contents; needs a human `git stash drop stash@{0,1,2}` (or `git stash clear`) to actually remove them.
