...
- **Query-testing methodology sweep — incomplete, 73 of 99 grammars remaining** (verify this count against the "Remaining for future batches" list further down before continuing — it was accurate as of batch 3). `docs/query-testing-methodology.md` was validated on Rust, then applied in three batches (batch 1: python/typescript/javascript/go/java/c/cpp/ruby; batch 2: c-sharp/php/swift/kotlin/scala/haskell/elixir/sql; batch 3: json/yaml/toml/html/css/bash/lua/markdown/dockerfile). Why it matters: every single language examined so far had real, previously-silent extraction bugs, several severe (C#'s `@type.reference` matching every identifier in the file; Haskell/Elixir complexity wildly inflated or wrong for every function; Python's `@definition.constant` matching nothing at all; JSON dropping every empty-string-keyed pair; CSS's `@font-face`/`@layer`/`@property`/`@container`/`@page`/`@scope` all invisible to symbol extraction). Open question: whether to continue a full sweep across all 99 grammars or stop at a high-value subset — the user chose "full sweep" earlier, but that predates knowing each batch costs roughly 3-4M tokens (3 batches of 8-9 languages each have been run so far). Next session should confirm this tradeoff is still the user's preference before committing to more batches.
  ...
  **Batch 3 process note (2026-08-07):** run with 9 parallel sub-agents sharing one checkout (no worktree isolation — ruled out for disk-write cost), each instructed to commit via either `git commit -- <explicit paths>` or a private `GIT_INDEX_FILE`. Two independent agents (HTML, Dockerfile) hit a serious variant of the index race not anticipated by that guidance: `GIT_INDEX_FILE=/tmp/normalize-idx-<lang>-$$` with the `export`+`git add` and the `git commit` split across separate Bash tool calls — each Bash invocation is a new shell with a different PID, so `$$` resolves to a *different* path per call, producing an empty private index and a commit that force-landed an empty tree directly on `master` (deleting the ~2728-file repo in one commit, twice, independently). Both were caught immediately by the agents themselves (one via `git show --stat` looking wrong, one via `git reflog`) and fixed with `git reset --soft` to the correct parent — this only moves the branch pointer, never touches the working tree or `.git/index`, so no work was lost either time, but it was close. **Lesson for future batches: never split a `GIT_INDEX_FILE=...$$` sequence across multiple Bash calls; either run the whole export+add+commit as one call, use a fixed literal path, or (simplest) just use `git commit -- <explicit paths>` and skip GIT_INDEX_FILE entirely.** Also mid-batch, a concurrent agent independently rewrote `scripts/pre-commit`/`.git/hooks/pre-commit` to scope fmt/clippy to staged packages instead of full-workspace (`05a0610a`, `8aceef3e`) — this was a real fix for a real hazard (one agent's in-progress unrelated edits elsewhere blocking another's unrelated commit) but means the hook no longer proves the whole workspace builds; run the full `cargo clippy --all-targets --all-features -- -D warnings` and `NORMALIZE_REQUIRE_GRAMMARS=1 cargo test -q --workspace` explicitly at the end of a batch rather than trusting the hook covered it. Confirmed clean (139 test-result blocks, 0 failed) after batch 3. **✅ Cheaper alternative DONE (2026-08-14, `b577d0a8`):** the shared-index race recurred roughly five more times in subsequent sessions despite the above guidance, so CLAUDE.md now also documents staging via `git hash-object -w` + `git update-index --cacheinfo` against a private `GIT_INDEX_FILE` (then committing with that same `GIT_INDEX_FILE`) — this touches neither the shared index nor the working tree, avoiding both the index race and the worktree-isolation cache cost. Explicitly noted that plain `git add` under a private `GIT_INDEX_FILE` is NOT sufficient, since it still reads the (possibly concurrently-mutating) working tree.

- **`extract_fixtures` fail-fast masks multi-failure situations — fixed.** The `normalize-facts` fixture test used to panic on the first mismatch it found (`find_fixture_cases()` sorts alphabetically, so whichever language sorted first ate the panic), so a single run reported one failure when many may exist. This caused real misdiagnosis twice: an apparent "ada bug" turned out to be all 40 fixture languages having a stale `SUMMARY.md`-derived expectation (ada merely happened to sort first alphabetically), and separately, fixing `c/add_numbers`'s header-guard bug unmasked four more already-stale fixtures (cpp, haskell, gleam, lua), and fixing clojure unmasked three more (hcl, objc, prolog) — each round costing a full investigate-fix-rerun cycle. Fixed in `crates/normalize-facts/tests/extract_fixtures.rs`: the JSON/text diff helpers now return `Result<(), String>` instead of panicking, the main loop collects every mismatch across every case (still printing each diff to stderr as encountered), and a single final `panic!` reports how many cases/languages failed (with the full case list) — collapsed to a one-line message when only one case fails, to avoid noise in the common case. `UPDATE_FIXTURES=1` regeneration path is unchanged. A survey of sibling fixture-driven tests found `normalize-cfg`'s snapshot tests (one `#[test]` per fixture, not a shared loop), `normalize-syntax-rules/tests/rule_fixtures.rs`, and `normalize-languages/tests/query_compile.rs` already collect-and-report correctly. One sibling with the same defect was found but left unfixed (out of this task's scope): `crates/normalize-refactor/tests/cross_file.rs`'s `module_resolver_coverage_matrix` (~line 712/758) asserts fail-fast inside two loops over language/resolver-expectation pairs; the fix is mechanical (same shape as the `extract_fixtures` fix). **✅ Follow-up DONE (2026-08-14):** fixed in `2c802080` — collect-and-report, one-line summary when a single language fails. While fixing it, found the matrices were actually vacuous: every entry was gated by `#[cfg(feature = "lang-*")]`, but `normalize-refactor`'s own `Cargo.toml` declares no such features, so every guard silently evaluated false and the test had been passing by iterating over two empty arrays. Removed the dead guards (the crate depends on `normalize-languages` with its default `langs-all` feature, so every language is always available — same fix already applied in `normalize-cfg`'s `coverage_matrix.rs`); the test now actually exercises all 36 `has_resolver` and 17 `not_applicable` languages.

- **Common Lisp `loop` never produced a CFG loop node — fixed.** `commonlisp.cfg.scm`'s loop pattern matched `list_lit`-headed forms via `#match? "^(do|do\\*|dolist|dotimes|loop)$"`, but CL's `loop` macro parses as its own dedicated `loop_macro` grammar node, never as a `list_lit` — confirmed via `normalize syntax query` against a real `loop` form (zero matches under the old pattern). Same finding already documented in `commonlisp.complexity.scm` (19261a0a) but left unfixed there for `cfg.scm`, out of that task's scope. Fixed: added a dedicated `(loop_macro) @cfg.loop` pattern and removed the dead `loop` alternative from the `list_lit` regex. `loop_macro` has no single condition to expose as `@cfg.loop.condition`, so it renders with the builder's existing unconditional-loop shape (same as Rust's `loop`) — cross-checked against `dolist`'s conditional-loop shape and TypeScript's labeled-edge loop snapshot. No CFG snapshot tests exist for Common Lisp (only `coverage_matrix.rs` classifies it as HAS_CFG), so verification was via `normalize cfg` output directly rather than a snapshot diff. `commonlisp::tests::unused_node_kinds_audit` still passes unchanged — that audit only scans `tags.scm`, and its `documented_unused` list already carried a comment noting `loop_macro` is referenced by `complexity.scm`/`cfg.scm` from outside that scope.
...
- **✅ DONE (2026-07-28): Orphaned `git stash` triage — all 3 entries verified obsolete via content-tracing and dropped with user approval.** `git stash list` is now empty. Historical record of what each contained:
  ...
  - `stash@{1}` → shipped as `f66961c5` (feat(sessions): parallelization hints, file edit heatmap, cost breakdown)
  - `stash@{2}` → shipped as `44617616` (feat(rules): add config-show command), later unified into `config show --section rules` (`25cb24ab`)

- **cmake fixture gitignore trap (2026-07-28): verified NON-ISSUE on current master — `e7379be6` fix confirmed durable, no other fixtures affected.** `crates/normalize-languages/tests/fixtures/cmake/CMakeLists.txt` is tracked (`git ls-files --error-unmatch` succeeds) and `git check-ignore -v` on it returns no match (the blanket `*.txt` rule in `.gitignore` no longer swallows it — `!**/tests/fixtures/cmake/CMakeLists.txt` exception added in `e7379be6`, confirmed present and correctly ordered after the blanket rule). The agent that hit the trap earlier the same day was necessarily working from a worktree checked out before `e7379be6` landed. Swept for the same bug class elsewhere: `git status --ignored --porcelain` across the whole repo, filtered to fixture/test paths, returns nothing — no other test fixture is currently caught by a blanket ignore rule (`*.txt` is the only such blanket rule in `.gitignore`; no `*.log`/`*.tmp`/etc. blanket rules exist). No code change needed; closing this as a tracked TODO item — the earlier report was accurate for its worktree's git state at the time, not a regression.

- **Stale `.claude/worktrees/` cleanup (2026-07-28): DONE for the safe cases, 2 branches PENDING USER REVIEW.** Audited the 16 leftover `worktree-agent-*` worktrees/branches (a merge agent earlier the same day had already merged and removed 3). Verified each remaining one two ways: `git merge-base --is-ancestor <branch> master` for unmerged commits, plus a `git archive <branch> | tar -x` snapshot diffed (`diff -rq`, excluding `.git`/`target`/`node_modules`/`.direnv`) against the live worktree directory to catch uncommitted changes that never even reached a commit. **13 were fully merged with zero working-tree drift** (the only diffs were gitignored `.claude/settings.local.json`, `.normalize/*.sqlite`, `web/sessions/dist`) — removed via `git worktree remove` + `git branch -d` (never `-D`): `a023fbbf9599232d2`, `a05dce261f0eddefa`, `a2458f2606ff633a7`, `a4beb6c18b8b53c5d`, `a5e1bd4b650cda1fa`, `a6958209bc163138f`, `a97a8f8a7db260dd1`, `abd1eaf5c01723635`, `ac57f7141b684b6ca`, `acc668267a8ec4606`, `ad606e9ee4678a2d5`, `ae0a8f6b99220969f`, `afc9da0d00ba65f3e`. **2 branches left untouched — PENDING USER REVIEW, do not delete without a decision:**
  - `worktree-agent-a279f98025bdaec11` (07f35b22, unmerged): `test(cli): B0 guide-link regression test + guide staleness fixes + CLAUDE.md crate count` — adds `crates/normalize/tests/guide_links.rs` (186 lines), touches `guide.rs`, several `SUMMARY.md` files, `CHANGELOG.md`, `TODO.md`. Looks like real, substantive finished test/doc work related to the B0 CLI-taxonomy thread below; no uncommitted drift on top.
  - `worktree-agent-ac628c38cdffd34dd` (6d586be9, unmerged): `feat(sync): incremental sync — skip unchanged files by mtime+size` — reworks `crates/normalize/src/commands/sync.rs` (187 lines) and `service/mod.rs` (60 lines). Substantive unmerged feature work; no uncommitted drift on top.
  Two other worktrees were skipped entirely as out of scope (in active use, not stale): the caller's own (`a0db4507f4b4c8c37`) and a concurrently-running unrelated session (`a31e3acc9ea920732`, locked, same-day mtime). Several additional worktrees not on the original stale list appeared mid-audit (`a07ebb41bad332648`, `a231dcbefa3500574`, `a59afebdb58240c3a`, `a7bb797e9d8dc0f07`, `a7f21d4528d6c2c3a`, `a80a988c7885083e2`, `abc4e1b59db734054`) — left untouched, out of scope for this audit.
...
- **Query-file compile guardrail (2026-07-27)**: The `all_registered_queries_compile` test in `crates/normalize-languages/tests/query_fixtures.rs` found 59 of 474 registered `.scm` query files silently fail `tree_sitter::Query::new()` against their real grammar — same bug class as the Lua fix in af9dbc0b (`GrammarLoader::get_compiled_query()` does `.ok()?`, so a broken query quietly returns zero results forever, never an error). ✅ DONE: the 13 non-`.cfg.scm` files fixed and verified with real capture output (`gleam.{tags,calls,complexity,imports}.scm`, `caddy.imports.scm`, `dockerfile.imports.scm`, `erlang.calls.scm`, `fsharp.calls.scm`, `groovy.types.scm`, `haskell.calls.scm`, `matlab.imports.scm`, `r.calls.scm`, `thrift.imports.scm`) — see CHANGELOG `[Unreleased] Fixed`. Of the 46 `.cfg.scm` (control-flow-graph query) failures: ✅ **Batch 1 DONE (2026-07-27): 10 languages fixed** — `ada`, `agda`, `awk`, `bash`, `c-sharp`, `cmake`, `d`, `dart`, `elixir`, `elm` — verified with real capture output via `normalize syntax query`/`normalize syntax ast`, fixtures extended for Bash/C#/D/Dart/Elixir where the sample lacked the needed control-flow constructs. See CHANGELOG `[Unreleased] Fixed`. ✅ **Batch 2 DONE (2026-07-27): 9 languages fixed** — `erlang`, `fish`, `fsharp`, `gleam`, `glsl`, `go`, `groovy`, `haskell`, `hcl` — verified with real capture output via `normalize syntax query`/`normalize syntax ast`, fixtures extended for all nine where the sample lacked the needed control-flow constructs; Gleam confirmed to have no if/else-expression node in this grammar version at all (case-only, documented rather than guessed). The Go fix also un-skipped `normalize-cfg`'s `go_cfg` snapshot tests (previously silently skipped with "go grammar not installed" because the query never compiled) — snapshots updated to real output via `cargo insta accept`. See CHANGELOG `[Unreleased] Fixed`. ✅ **Batch 3 DONE (2026-07-27): 9 languages fixed** — `idris`, `julia`, `kotlin`, `lean`, `lua`, `matlab`, `meson`, `objc`, `ocaml` — verified with real capture output via `normalize syntax query`/`normalize syntax ast` (MATLAB required a scratch `GrammarLoader`-based probe test instead, since `.m` extension collision with Objective-C makes the CLI resolve `fixtures/matlab/*.m` to the wrong grammar — see the newly-flagged bug below). Fixtures extended for Idris (case expression), Lean (match expression + do-notation for-loop), and Meson (elif/else/foreach/break/continue) where the sample lacked the needed constructs. Lean's grammar has no while-loop node type at all and no break/continue/throw nodes (do-notation only models `do_return`) — documented as absent. Objective-C's C-based grammar accepts fast-enumeration syntax (`for (T *x in xs)`) but does not populate `for_statement`'s normal fields for it (flat, field-less parse) — left uncaptured rather than fabricated, documented in the query file. ✅ **Batch 4 DONE (2026-07-27): 9 languages fixed** — `perl`, `php`, `powershell`, `prolog`, `rescript`, `ruby`, `scala`, `starlark`, `svelte` — verified with real capture output via `normalize syntax query`/`normalize syntax ast`. Perl and Prolog both claim the `.pl` extension (CLI resolves it to Prolog), so Perl was probed through `.pm`-renamed fixture copies — flagged as a pre-existing collision, not fixed here (same class of bug as the MATLAB/Objective-C `.m` collision below). Perl's `conditional_statement` uses `condition`/`block` fields (not `consequence`/`alternative`) with a recursively-nested `elsif`/`else` chain (each nested one level inside the previous, not siblings); `last`/`next`/`redo` are `(loopex_expression loopex: "last"|"next"|"redo")`, not their own node types; `die` is an ordinary function call matched via `#eq?`, same as Lua's `error()`. PHP's `match_expression` arms nest `match_condition_list` one level deeper than expected, and `throw_statement` never existed (only `throw_expression`). PowerShell has no `body` field anywhere (switch/for/foreach/while/do bodies are all unnamed positional `statement_block`s) and `return`/`break`/`continue`/`throw` are all `(flow_control_statement <keyword>)`, not separate node types. Prolog's grammar is a generic term reader with no `if_then`/`call` node types at all — if-then-else and `catch`/`throw` are matched via operator text (`->`, `;`) and functor-name `#eq?` on generic `operator_notation`/`functional_notation` nodes; fixture extended with if-then-else, catch/3, and throw/1 examples since the original had none. ReScript has no `return_expression` node type at all (functions return their last expression implicitly, genuinely absent, documented not fabricated) and `if_expression`/`switch_expression` have no named fields. Ruby's `case/in` pattern matching (3+) uses a distinct `case_match`/`in_clause` node pair (not `case`/`in_pattern`); `begin`/`rescue`/`ensure` have no `body` field (fixed a real bug where the try-body was never captured at all, 0 matches, despite the query having compiled fine before); `for`'s iterable is field `value` not `pattern` (fixed a real semantic bug — the original captured the loop variable as the loop condition); bare `raise` (re-raise) is an unadorned `(identifier)`, matched via `#eq?`. Scala's `match_expression` body is `case_block` not `match_block`; Scala has no break/continue keywords or node types at all — non-local loop exit is via the opt-in `scala.util.control.Breaks` library calls, judged too false-positive-prone to match via `#eq?` (unlike Perl/Ruby's universal builtins) and left uncaptured, documented. Starlark's `if_statement`/`elif_clause` then-arm field is `consequence`, not `body`. Svelte's template-directive `if_statement`/`each_statement` have no `condition`/`consequence`/`body` fields at all — condition is raw unparsed text nested in `if_start`/`each_start`, and then/else bodies are flat unnamed siblings with no wrapper node, so only the first sibling is captured (anchored with `.`) — a faithful-but-partial signal for multi-element branches; confirmed Svelte's `<script>` JS logic is intentionally out of scope for this template-only query (not a delegation bug). See CHANGELOG `[Unreleased] Fixed`. ✅ **Batch 5 DONE (2026-07-27, FINAL BATCH): 9 languages fixed** — `swift`, `tlaplus`, `vb`, `verilog`, `vhdl`, `vim`, `vue`, `zig` — verified with real capture output via `normalize syntax query`/`normalize syntax ast`; fixtures extended for TLA+, Verilog, VHDL, Vim, Vue, and Zig where the sample lacked the needed control-flow constructs. Swift's `if_statement` has no `else_clause` wrapper (else is a bare named `else` node followed by either a nested `if_statement` or a `(statements)` block) and return/break/continue are anonymous tokens inside a shared `control_transfer_statement` (throw is the named `throw_keyword`). TLA+'s `if_then_else` field is `if`, not `predicate`. VB's branch/loop bodies are plain `(statement)` siblings with no `body:` field, `for_statement` has no `for_to_clause` wrapper, the Do/Loop node is `do_statement` not `do_loop_statement`, and try/catch/finally use `catch_block`/`finally_block` not `*_clause`. Verilog's `cond_predicate`/`case_expression`/`statement_or_null`/`for_initialization`/`for_step` are bare named nodes, not fields, and return/break/continue are anonymous tokens inside a shared `jump_statement`. VHDL's `if_statement` has no `condition:` field or `elsif_sequence_of_statements`/`else_sequence_of_statements` wrapper nodes — instead repeatable `(if ...)`/`(elsif ...)`/`(else ...)` child nodes each wrap their own `conditional_expression`/`sequence_of_statements`; `loop_statement` wraps a bare `(for_loop)`/`(while_loop)` (no `iteration_scheme`). Vim's branch/loop body is a named `(body)` node not a `body:` field, and `for_loop`'s iterable is field `iter:` not `variable:`. Vue's directive attributes have no `directive_argument` node — the condition/for-head is raw unparsed text at `(directive_attribute (quoted_attribute_value (attribute_value)))`, and `v-else-if`/`v-else` are disambiguated from `v-if` via `#match?` on the directive name text (confirmed template-directive-only scope, matching the Svelte precedent, before writing the query). Zig has no `condition:`/`body:` fields anywhere, no `ElseSuffix`/`ReturnStatement`/`BreakStatement`/`ContinueStatement` node types (return/break/continue are anonymous tokens inside a shared `AssignExpr`), and its `catch` error-handler is the `right:` field of a `BinaryExpr` whose operator is a `BitwiseOp` wrapping `catch` (no dedicated try/catch node). **`zsh` fixed at the schema level only, NOT verified at runtime** — see the flagged grammar defect below; the query file documents this prominently rather than claiming false confidence. This closes the 46-file `.cfg.scm` remediation effort (batches 1–5, all originally-broken files now fixed) plus the 13 non-`.cfg.scm` files from the initial pass — **59 of 474 originally-broken query files are now all fixed and compiling clean**.

**Guardrail landed for real:** `all_registered_queries_compile` in `crates/normalize-languages/tests/query_fixtures.rs` (already committed as of 51730d4c, not actually "carried forward uncommitted" as earlier notes here assumed) now passes with 0 failures across all 474 registered query files — confirmed via `cargo test -p normalize-languages --test query_fixtures all_registered_queries_compile -- --nocapture`.

**Newly flagged (out of scope for this remediation, needs its own investigation): the `arborium-zsh` 2.17.0 grammar cannot parse real-world zsh source at all beyond trivial single-word input.** Verified by parsing a dozen minimal snippets with `normalize syntax ast` against a freshly-forced rebuild (`cargo xtask build-grammars --force`) of `target/grammars/zsh.so`: even `echo hi` (a bare two-word command) fails to parse as a single `command` node — it splits into an ERROR-wrapped first word and an unrelated second `command` node for the second word. `if`/`for`/`while`/`case` keyword-led statements never produce `if_statement`/`for_statement`/`while_statement`/`case_statement` nodes from any realistic source tried (with/without brackets, `[ ]` vs `[[ ]]`, nested/top-level, single/multi-line). `grammar.json` declares `"externals": []` — no external scanner — consistent with the reserved-word/argument disambiguation that `tree-sitter-bash` normally handles via a scanner simply not being present in this grammar. This affects ALL zsh query types (tags, imports, calls, complexity — not just CFG), so `zsh.cfg.scm` was fixed to be schema-correct (every node type/field checked against `node-types.json`, modeled on the empirically-verified `bash.cfg.scm`) so it compiles, but its *runtime* capture behavior is unverified and, per the evidence gathered, unlikely to ever fire until the grammar itself is fixed or replaced upstream. Needs separate investigation — not a `.cfg.scm`-authoring fix. Also note: while investigating, `~/.config/normalize/grammars/*.so` was found to be a stale (2026-05-08) copy that the CLI's default `GrammarLoader::new()` search path prefers over a locally-rebuilt `target/grammars/`; set `NORMALIZE_GRAMMAR_PATH=target/grammars` when testing local grammar changes via the `normalize` CLI, or the CLI will silently exercise the old `.so`.

**Follow-up investigation (2026-07-27, later session): root cause pinpointed — arborium's packaged crate is missing the upstream scanner, and no version bump fixes it. Confirmed purely upstream, not fixable in this repo.** Independently reproduced the defect from a clean `cargo xtask build-grammars --force`: `normalize syntax ast` on a two-line `echo hi` / `if [[ 1 -eq 1 ]]; then echo yes; fi` fixture (`NORMALIZE_GRAMMAR_PATH=target/grammars`) produces a flat `program` node whose children are mostly bare `word`/`file_descriptor` tokens plus 5 `ERROR` nodes (one on `[[` itself) — no `command` node wraps `echo hi`, no `if_statement` appears at all, matching the prior report exactly (worse than "single-word only": even the two-word case is unparsed). Checked crates.io: **`arborium-zsh` has a newer version, 2.18.1** (we're pinned to `2.17.0` via `arborium-zsh = "*"` in `crates/normalize-grammars/Cargo.toml`, but `Cargo.lock` resolved 2.17.0). Downloaded both via `cargo info`/`cargo cargo-package-cache` and diffed the extracted sources (`~/.cargo/registry/src/.../arborium-zsh-{2.17.0,2.18.1}`): **`grammar/src/grammar.json`, `parser.c`, `node-types.json`, `queries/`, and `samples/` are byte-identical between the two versions** — only `build.rs` (WASM `llvm-ar` toolchain handling), `Cargo.toml`, and a build hash differ. So 2.18.1 is not a candidate fix; the grammar itself hasn't changed since 2.17.0 and version-bumping would do nothing. The crate's own README records its upstream provenance: `georgeharker/tree-sitter-zsh` commit `c50ded6979eb7aae8b8da5da9ac2e9e984804881`. **Root cause found by comparing against that exact upstream commit via the GitHub API**: upstream's `grammar.js` at that commit declares ~40 `externals` tokens (`variable_name`, `heredoc_start`/`_content`/`_end`, `test_operator`, `regex`, brace/dollar disambiguation tokens, etc.) — the same class of scanner-driven reserved-word/argument disambiguation `tree-sitter-bash` relies on — and upstream's `src/` directory at that commit contains a 91,929-byte hand-written `scanner.c` implementing them. **But the vendored `arborium-zsh` crate (both 2.17.0 and 2.18.1) ships `"externals": []` in its `grammar.json` and has no `scanner.c` anywhere in the package** (`build.rs` references an "optional, hand-written" `scanner.c` that simply isn't present). Without that scanner, the parser has no way to disambiguate keywords (`if`/`for`/`while`/`case`) from plain words or to delimit command arguments — which is exactly the observed failure mode (bare `word` tokens instead of `command`/`if_statement`/etc.). This means arborium's grammar-generation pipeline (`cargo xtask gen --name zsh` per its own README) drops the external scanner for this specific grammar during packaging — a packaging defect in `bearcove/arborium`, not a defect in `georgeharker/tree-sitter-zsh` upstream, and not something this repo can route around by pinning a different `arborium-zsh` version (none published contains the scanner) or a different query. **Conclusion: purely upstream (arborium), not fixable in this repo.** No code change made — pin left at `arborium-zsh = "*"` (resolves to 2.17.0), `zsh.cfg.scm`'s existing "unverified at runtime" caveat stands confirmed rather than resolved. Should be reported to `bearcove/arborium` (checked `github.com/bearcove/arborium/issues` for an existing "zsh" report first — found none) with the scanner.c-missing evidence above; until arborium fixes their zsh packaging, all zsh query types (tags/imports/calls/complexity/cfg) will continue to produce near-zero real captures on realistic source, not just CFG. **✅ DONE (2026-07-27): dev-workflow staleness warning added.** Root cause: `~/.config/normalize/grammars/` is populated once (via `normalize grammars install`, downloading a GitHub release) and only ever re-checked against the running binary's *version string* (`ensure_grammars_first_use` in `crates/normalize/src/commands/grammars.rs`) — that check exists to catch a stale install after `normalize update`, but has no signal for a developer who edits grammar sources and reruns `cargo xtask build-grammars` (writing to `target/grammars/`, per its own doc comment — it never touches the config dir) without the crate version changing. `NORMALIZE_GRAMMAR_PATH` was always the intended override for local testing (documented in `grammar_loader.rs`'s search-order doc comment and `skeleton.rs`), but nothing warned when it was left unset while a newer local build existed, so the CLI silently kept exercising the old `.so`. Fixed with a scoped, dev-only nudge rather than a new subsystem: `commands::grammars::warn_if_dev_grammars_stale()` (called from `main.rs` right after the existing first-run grammar check) is a no-op unless `NORMALIZE_GRAMMAR_PATH` is unset *and* a `./target/grammars/` directory with `.so`/`.dylib`/`.dll` files exists relative to cwd (i.e. only fires inside a workspace checkout — never for installed end-user binaries); when both are true it compares mtimes against the same-named file in the config dir and, if any local build is newer, prints one stderr warning naming the stale files and both remediation commands (`NORMALIZE_GRAMMAR_PATH=target/grammars` or `normalize grammars install --force`). Verified against the real stale `~/.config/normalize/grammars/` on this machine (May 8 vs a July 27 `cargo xtask build-grammars` rebuild) — the warning fired correctly listing all 98 stale grammars, and was silent once `NORMALIZE_GRAMMAR_PATH` was set. `cargo clippy --all-targets --all-features -p normalize -- -D warnings` and `cargo test -p normalize -q` both clean.

**Re-verification (2026-07-27, later session): prior "missing scanner.c" finding challenged by the user (upstream's `src/scanner.c` clearly exists on GitHub `main` today) — re-checked rigorously, finding confirmed correct.** The doubt was reasonable to raise (a claim of "file missing upstream" is easy to get wrong if checked against a moving branch), so this re-check used the exact pinned commit rather than `main`: `curl -sI https://raw.githubusercontent.com/georgeharker/tree-sitter-zsh/c50ded6979eb7aae8b8da5da9ac2e9e984804881/src/scanner.c` → **HTTP 200**, and downloading it gives exactly **91,929 bytes**, matching the prior report's cited size precisely — upstream did have the scanner at that pinned commit, confirming the original citation was accurate (the user's observation that it's on `main` today is also true and not in conflict — both can hold). Then re-extracted the actual installed packages fresh: `~/.cargo/registry/src/*/arborium-zsh-2.17.0` and `-2.18.1` (paths taken from `Cargo.lock`, not guessed), and ran `find <pkg-dir> -type f` (full recursive listing of every file in the package, not just an `-iname 'scanner*'` guess) — the complete file list for 2.17.0 is: `.arborium-hash`, `build.rs`, `Cargo.lock`, `Cargo.toml(.orig)`, `grammar/src/{grammar.json,node-types.json,parser.c,tree_sitter/{alloc.h,array.h,parser.h}}`, `queries/highlights.scm`, `README.md`, `samples/git.plugin.zsh`, `src/lib.rs` — **no `scanner.c` or `scanner.*` anywhere in the package**, at any path, under any name. `grammar/src/grammar.json`'s `"externals"` key is verified `[]` (empty array, confirmed by direct grep of the packaged file, not inferred). By contrast, the upstream `grammar.json` fetched at the same pinned commit has a non-empty `"externals"` array with 20+ named entries (`heredoc_start`, `simple_heredoc_body`, `variable_name`, `test_operator`, `regex`, etc. — the same scanner-driven disambiguation class as `tree-sitter-bash`). `build.rs` in the package still contains the comment `// scanner.c (optional, hand-written)` and an `.include(&grammar_dir)` call, but there is no `scanner.c` file for it to compile — consistent with the prior finding that arborium's packaging pipeline silently drops the hand-written scanner for this grammar. **Conclusion: prior finding stands, now verified more rigorously (exact-commit HTTP check + exhaustive recursive `find` across the full package tree, not a top-level guess).** No TODO correction needed on the substance; this entry exists to record that the re-check was performed and what evidence was collected, per the "write it down now" rule, since the user's challenge was itself a legitimate signal worth responding to with primary evidence rather than reassurance.

**DECISION (2026-07-28): NOT fixing locally. Blocked on upstream `bearcove/arborium`. Do not re-open this investigation — see below for what "done" means here.** Vendoring the missing `scanner.c` ourselves was considered and rejected: CLAUDE.md's "grammars come from arborium or us" policy means a scanner.c we didn't author, applied only to work around one grammar's packaging bug, is exactly the kind of per-language special case that policy exists to prevent — the fix belongs in arborium's packaging pipeline, not as a patched-in vendor file here. **✅ DONE (2026-07-28): blocked status made unmissable everywhere a user or agent could hit it**, so nobody re-investigates and nobody is misled into thinking zsh extraction works:
- `normalize_languages::known_broken_grammar(name)` (`crates/normalize-languages/src/grammar_loader.rs`) is the single source of truth — a `grammar name -> reason` registry, `zsh` the only entry so far.
- `normalize-facts`' `Extractor::extract_with_support` (`crates/normalize-facts/src/extract.rs`) prints a one-time-per-process stderr warning instead of silently handing back empty symbols — covers `view`, `structure rebuild`, `analyze`, and everything else that goes through the shared extraction path.
- `normalize cfg` (`crates/normalize-cfg/src/service.rs`) returns a hard `Err` with the reason rather than producing a garbage CFG — this is a single-invocation command, so a clear error beats a warning-then-garbage-output.
- `normalize syntax ast` (`crates/normalize/src/commands/analyze/ast.rs`) prints a stderr warning before showing the (broken) tree, since this is the exact debugging tool CLAUDE.md tells agents to reach for when investigating grammar behavior.
- `normalize grammars list` (`crates/normalize/src/commands/grammars.rs`) lists a "Known issues" section for any installed grammar with a known defect (both text and JSON via the new `known_issue` field on `GrammarEntry`).
- `README.md` and `docs/language-support.md` both call out the defect in prose, so it's visible without running any command.
- All five `zsh.*.scm` query files (`tags`/`calls`/`imports`/`complexity`/`cfg`) carry the same "unverified at runtime, known-broken grammar" header — they were made schema-correct but could never be runtime-verified, for exactly this reason.

**Upstream issue filed:** https://github.com/bearcove/arborium/issues/213

**Root cause (confirmed 2026-07-27):** `arborium-zsh` (both published versions, 2.17.0 and 2.18.1) ships `grammar.json` with `"externals": []` and no `scanner.c` file anywhere in the crate. Upstream `tree-sitter-zsh` (commit `c50ded6979eb7aae8b8da5da9ac2e9e984804881`, which arborium claims to package) declares ~40 external tokens in `grammar.js` and includes a 91,929-byte `scanner.c` implementing keyword/word/argument disambiguation — the same scanner-driven approach `tree-sitter-bash` uses. Without it, the grammar cannot parse even `echo hi` as a `command` node (splits into ERROR-wrapped first word + separate second node); `if`/`for`/`while`/`case` statements never produce their respective node types. The two published versions are byte-identical in their `grammar/src/{grammar.json,parser.c,node-types.json,queries/,samples/}`, so version-bumping won't fix it; this is a packaging-pipeline defect in `bearcove/arborium` (not a defect in upstream `tree-sitter-zsh` itself, which has the scanner). **Not fixable in this repo** — the fix belongs in arborium's grammar-generation pipeline.

Note: `normalize-cfg`'s `java_cfg`/`lua_cfg` snapshot tests and `normalize-facts`' `extract_fixtures` (ada) fail in this environment whenever grammars are actually built (pre-existing, unrelated to this CFG remediation — confirmed present on master before batch 2's changes via `git stash`, reconfirmed before batch 3, batch 4, and batch 5/final). **✅ All three now fixed**: `java_cfg` in `b851ecb2` (2026-07-27) and `lua_cfg` in the follow-up fix (2026-07-28) — both were snapshots recorded before their respective grammars were built locally, capturing the "grammar not installed"/"tags query incompatible" placeholder text; refreshed to real CFG output and cross-checked against already-verified sibling snapshots (go/java/python/js/ts loop and branch shapes) rather than blindly accepted. **`extract_fixtures` (ada) fixed 2026-07-28**: root-caused as a stale-expectation-file issue, not an extraction bug or test-harness bug — `c20e238e` ("remove SUMMARY.md convention") deleted all 446 in-repo `SUMMARY.md` files (including the per-fixture-project ones under `crates/normalize-facts/tests/fixtures/*/add_numbers/project/`) but never regenerated the corresponding `expected/symbols.json` snapshots, which still asserted a `SUMMARY.md`/`Heading` symbol that no longer exists on disk. This affected all 40 fixture languages identically, not just ada — ada only surfaced because `find_fixture_cases()` sorts alphabetically and the test panics on the first mismatch, so every other language's identical staleness was masked. Verified via `git diff` after `UPDATE_FIXTURES=1 cargo test -p normalize-facts extract_fixtures` that the only changes across all 40 `expected/symbols.json` files were removal of the stale `SUMMARY.md` entries (plus a few `[\n]`→`[]` reformats where it was the sole entry) — no other symbol data changed, confirming no genuine `.scm`/extraction regression. `imports.json` fixtures were untouched (0 files). Full workspace `cargo test --workspace -q` (137 test-result blocks) and `cargo clippy --all-targets --all-features -- -D warnings` both clean afterward — master is fully green for the first time this cycle. **✅ FIXED (2026-07-27): `normalize-languages`' `lua::tests::unused_node_kinds_audit` was a stale-test false positive, not a real extraction gap.** The audit's tags.scm parser (`registry.rs::validate_unused_kinds_audit`) only recognizes a node kind as "used" when its opening paren starts a line — a deliberate simplification, not a bug worth widening (a broader scan matching `(kind` anywhere makes generic leaf kinds like `identifier`/`type_identifier` register as "used" in nearly every language's tags.scm, since `(identifier) @name` is a near-universal nested leaf capture, which defeats the check's purpose; confirmed by trying it — broke 35 other languages' audits). `method_index_expression` (the `Table:method(...)` definition form) was genuinely matched by `lua.tags.scm`'s query, just written inline after `name:` on the same line as `(function_declaration`, so the audit's line-start scan couldn't see it and flagged it as an undocumented gap. Reformatted the one pattern in `lua.tags.scm` so `(method_index_expression` opens its own line, matching the convention `dot_index_expression` already used in the same file. No query semantics or extraction behavior changed — verified via `normalize syntax query` against `tests/fixtures/lua/sample.lua` that `Stack:push`/`pop`/`is_empty` still resolve as `@definition.method`/`@name` before and after. `cargo clippy --all-targets --all-features -- -D warnings` and `cargo test -q -p normalize-languages` clean. **MATLAB and Objective-C both register the `.m` file extension in `normalize-languages`; the extension-based language registry picks objc for any `.m` file including MATLAB's own fixtures (`normalize view crates/normalize-languages/tests/fixtures/matlab/sample.m` reports `"grammar":"objc"`), so any CLI command that infers language from a bare `.m` path silently analyzes MATLAB source as Objective-C. Needs a real disambiguation strategy (shebang/content sniffing, or an explicit per-directory/config override) — not a CFG-remediation fix. Perl/Prolog share the same problem for `.pl` (documented above).**
✅ **FIXED (2026-07-27): layered language-detection resolver.** Re-derived the full collision list by grepping every `extensions()` return value in `crates/normalize-languages/src/*.rs` (98 files) — confirmed 5 pairs: `.m` (matlab/objc), `.pl` (perl/prolog), `.s`/`.S`/`.asm` (asm/x86asm — `.asm` itself also collides, not just `.s`/`.S`), `.conf` (ini/nginx), and `.scm` (query/scheme, already intentionally resolved via registration-order comment in `registry.rs`, left alone). `support_for_path`/`support_for_extension`/`extension_map()` are **unchanged** (still last-registered-wins, ~90 call sites across the workspace untouched — see below) so nothing regresses; a new parallel API in `crates/normalize-languages/src/registry.rs` does the real work: `candidates_for_extension`/`ambiguous_extensions()` derive the collision set live from registration data (self-updating — a newly added language that collides with an existing extension shows up automatically, no hardcoded list to keep in sync); `Language::sniff_hints()` (new trait method, default empty, `crates/normalize-languages/src/traits.rs`) lets each of the 8 colliding languages (matlab/objc/perl/prolog/asm/x86asm/ini/nginx) declare its own shebang patterns and weighted content-signal keywords — kept per-language, not as `if grammar_name == X` branches in the generic resolver, per the CLAUDE.md rule against language-specific branches in language-agnostic code; `resolve_language(path, content, explicit_lang, overrides)` layers explicit `--lang` → `.normalize/config.toml` `[languages]` glob overrides (new `LanguagesConfig` in `crates/normalize/src/config.rs`, `LanguageOverrides` in registry.rs) → shebang sniff (high confidence, decisive only if it names exactly one candidate) → weighted heuristic sniff (low confidence, decisive only on a unique strictly-positive max score — a tie or all-zero score is treated as genuinely inconclusive and is **not** guessed) → the documented last-registered-wins default, reporting every candidate that was in contention via `LanguageResolutionResult::describe()` (logged with `tracing::info!` whenever the extension was actually ambiguous). Wired into `normalize syntax ast` as the first concrete integration (`--lang` flag, `crates/normalize/src/service/syntax.rs` + `crates/normalize/src/commands/analyze/ast.rs::build_ast_output`) — chosen because it already reads file content before resolving language and has exactly one call site, making it low-risk; verified end-to-end (`--lang matlab` override, `.normalize/config.toml` `[languages]` override, and unguided content-sniffing to objc all produce the correct `tracing::info!` message and correct parse). The other ~90 `support_for_path`/`support_for_extension` call sites (indexer/walker filters in `normalize-facts`, `normalize-native-rules`, etc.; single-file commands under `commands/view/*`, `commands/analyze/*`; refactor/edit call sites) were deliberately left on the unchanged legacy API — threading `--lang`/config-aware resolution through all of them, especially `view` (20+ params, dispatches through `build_view_service` into `file.rs`/`symbol.rs`/`lines.rs`/`search.rs`, each with its own `support_for_path` call), is a genuinely separate, larger integration decision flagged here rather than guessed at. 12 new unit tests in `crates/normalize-languages/src/registry.rs` (`resolver_tests` module) cover: config override beats sniffing, glob-pattern path matching, shebang resolving both Perl-over-Prolog and Prolog-over-Perl, heuristic sniffing resolving both MATLAB-over-ObjC and ObjC-over-MATLAB, a genuinely-ambiguous near-empty file *not* being silently guessed (asserts `AmbiguousDefault` + both candidates surfaced + message contains "ambiguous"), and that the deterministic fallback for all 5 documented pairs matches the pre-existing `support_for_path` "winner" (regression guard). `cargo clippy --all-targets --all-features -- -D warnings` and `cargo test -q` clean (workspace default-members + full `-p normalize-languages` suite, 331+192 tests). See CHANGELOG `[Unreleased] Added`.

- **✅ DONE (2026-07-27): GrammarLoader compile-failure visibility + downstream call-site audit (task #6, follow-up to the 59-query remediation above).** The 59-query fix repaired the broken `.scm` files, but the root cause — `GrammarLoader::get_compiled_query()` swallowing `tree_sitter::Query::new(...)` failures into a bare `None`, indistinguishable from "language legitimately has no query for this purpose" — was still live, so a *future* broken query would silently regress to zero results with no signal, same as Lua did. Fixed at the source: `get_compiled_query` now logs `log::error!` the first time a query fails to compile (deduped via a new `failed_query_cache`, so it logs once per grammar/query-type rather than once per file processed) and records the error for introspection via the new `GrammarLoader::query_compile_error(grammar, query_type)`. Return type stayed `Option<Arc<Query>>` (lower-risk than threading a `Result` through every caller — justified because callers only ever reach `get_compiled_query` after already confirming a query string exists via `get_tags`/`get_calls`/etc., so a `None` from `get_compiled_query` itself is *always* a compile bug, never a legitimate absence — verified this invariant holds at all 6 pre-existing call sites before relying on it). Audited the 15-file list from the parent investigation (all still present, no drift) plus every other direct `tree_sitter::Query::new(...).ok()`-style call bypassing the loader (found via full-workspace grep, not just the seed list): routed all of them through `get_compiled_query` so they get both the new logging and the loader's existing compiled-query cache (several were recompiling the same query on every single file — a latent perf bug fixed as a side effect). Touched: `normalize-facts/src/index.rs` (CFG tags query), `normalize-deps/src/lib.rs` (tags + imports), `normalize-edit/src/lib.rs` (tags), `normalize-budget/src/metrics/complexity_delta.rs` (tags + complexity), `normalize-ratchet/src/metrics/{file_stats,complexity,call_complexity}.rs` (tags + complexity + calls), `normalize-native-rules/src/{long_function,high_complexity}.rs` (tags + complexity), `normalize-git-history/src/complexity.rs` (tags), `normalize-scope/src/lib.rs` (locals), `normalize/src/tree.rs` (highlights + injections — this one had its *own* duplicate local query cache, removed entirely in favor of the loader's), `normalize/src/analyze/{function_length,complexity}.rs` (tags). Two call sites already had exemplary `Result`-based error surfacing before this change and needed no fix: `normalize-cfg/src/{builder,service}.rs` (`BuildError::QueryCompile`/`format!("failed to compile...")`, properly threaded to the CLI) and `normalize-syntax-rules/src/query.rs` (`Result<_, String>` for user-authored ad-hoc queries). `normalize-syntax-rules/src/runner.rs`'s rule-compilation code (per-language/cross-language *rule* queries, not `GrammarLoader`-sourced `.scm` files) was reviewed and left alone — its per-grammar compile-failure filtering is intentional applicability filtering for cross-language rules (a rule may not apply to every grammar it's tagged for), a different domain from the GrammarLoader case, already has partial `eprintln!` visibility for the terminal failure case; flagged as a separate, lower-priority thing rather than folded into this change. No CLI exit-code/control-flow changes — every touched call site kept its existing `Option`/fallback semantics; visibility is now via `log::error!` (this codebase's established convention for this class of non-fatal-but-should-be-loud issue, per `normalize-facts/src/index.rs`'s existing `tracing::warn!` pattern) plus the new introspection method. Added `test_get_compiled_query_surfaces_compile_failure` and `test_get_compiled_query_success_leaves_no_error` in `grammar_loader.rs` — inject a deliberately-invalid query (references a non-existent node type) against the real `rust` grammar and assert the failure is retrievable via `query_compile_error`, not just swallowed; a real compile-error message is checked byte-for-byte for a distinguishing substring, not merely "returned None". `cargo clippy --all-targets --all-features -- -D warnings` clean; `cargo test -q` clean modulo three pre-existing environment-dependent failures unrelated to this change (`normalize-facts::extract_fixtures` ada SUMMARY.md heading, `normalize-cfg::java_cfg` stale snapshots now that the java grammar is actually built, `normalize-languages::lua::tests::unused_node_kinds_audit` — all reconfirmed pre-existing via `git stash`). See CHANGELOG `[Unreleased] Changed`. (All three subsequently fixed — see the `extract_fixtures` (ada) entry above and its neighbors for `java_cfg`/`lua_cfg`.)

- **Query extraction completeness — prototyped on Rust, sweep in progress across 99 registered grammars.** Compiling clean (the 59-query remediation above) doesn't mean a query is *complete*: it can still match only some of the node-type variants a grammar's `node-types.json` allows for a given field, same bug class as the original Lua fix. Cross-referenced `rust.{tags,calls,imports}.scm` against `node-types.json` field-by-field and found (and fixed) four real, previously-silent gaps: turbofish calls (`generic_function`) entirely unmatched in both `calls.scm` and `tags.scm`; generic/path-qualified impl blocks (`impl<T> Foo<T>`, `impl std::fmt::Display for Foo`) losing their container in `tags.scm`; `use path::{self, Other};` self-imports dropped in `imports.scm` (present in the pre-existing `sample.rs` fixture itself, never caught because the old test only checked for `HashMap`); bare wildcard imports (`use path::*;`, the common form) unmatched because only the rarer braced form (`use path::{*};`) was handled; scoped calls (`Type::method()`) missing from `tags.scm`'s `@reference.call` (already present in `calls.scm` but never ported). All fixed and covered by new completeness/negative-case tests in `query_fixtures.rs` (`rust_*_completeness_*`, `rust_*_negative_*`) plus an extended `sample.rs` (closures, generics, nested modules, iterator chains, multiple trait impls) and a new `variants.rs` fixture (one construct per grammar-legal field variant, plus documented negative cases). The reusable methodology — node-types.json lookup, per-field variant enumeration, `normalize syntax query` verification, fixture/test structure — is written up at `docs/query-testing-methodology.md`.

  **✅ Batch 1 DONE (2026-07-28): Python, TypeScript, JavaScript, Go, Java, C, C++, Ruby** — all 8 applied via parallel worktree-isolated agents, each merged back with conflicts resolved by hand (mainly the shared `query_fixtures.rs` test-helper additions and a cmake fixture collision — see below). Real, previously-silent bugs found and fixed per language (full detail in `CHANGELOG.md` `[Unreleased] Fixed`):
  - **Python**: `@definition.constant` never matched any module-level constant (`expression_statement` is a supertype alias for `assignment`, not a real wrapping node — the old query could never match); subscript-dispatched calls (`handlers[event]()`) unmatched in calls/tags; `python.types.scm` missing `generic_type`/dotted-subscript generics, PEP 604 unions (`binary_operator`, not the schema's `union_type`), PEP 695/646/612 variadic/paramspec params, `Callable[[...], ...]`, and multi-segment dotted annotations.
  - **TypeScript/JavaScript**: `namespace Foo {}` (the common `internal_module` node) invisible to tags (only the legacy `module` keyword was handled); private (`#foo()`) and computed (`[key]()`) methods dropped from tags/calls in both languages; namespaced constructors (`new ns.Foo()`) dropped from TS tags; `extends`/`implements` had zero reference captures in TS tags+types and JS tags; `import { default as alias }` produced zero captures (`default` is an anonymous token, not `identifier`); `import X = require(...)` and dynamic `import(...)` unhandled; several `call_expression.function` variants missing (`subscript_expression`, `parenthesized_expression`, `non_null_expression`, chained calls). TSX was checked and correctly left out of scope (diverges meaningfully in every query file except decorations).
  - **Go**: `type MyInt = int` type aliases entirely unhandled in tags/types (`type_alias` is a distinct node from `type_spec`); multi-name `const (A, B = iota, iota)` declarations lost every name after the first (only the first identifier in a comma-list is field-tagged); raw-string-literal import paths (`` import `pkg` ``) unmatched in all four `import_spec` forms. Documented (not fixed — no stable callee name exists) three deliberately-excluded `call_expression.function` variants: IIFEs (`go func(){}()`/`defer func(){}()`), curried calls, dispatch-table calls; also documented that explicit generic-instantiation calls parse as `type_conversion_expression`, not `call_expression`, in this grammar version.
  - **Java**: generic/path-qualified constructor targets (`new ArrayList<>()`, `new java.util.Date()`) unmatched in tags; generic/qualified superclass and interface tags lost (`extends AbstractList<String>`, `implements Comparable<Foo>`); `record`/`@interface` declarations entirely unhandled in tags+types; `super(...)`/`this(...)` explicit constructor invocations (a distinct node from `method_invocation`) unmatched in calls+tags; bare single-segment imports dropped, plus a duplicate-capture bug where an unconstrained plain-import pattern also matched every wildcard/static import (2-4x duplicate `@import` captures per statement).
  - **C/C++**: union definitions asymmetric with struct handling in both languages (bodyless usage mislabeled `@definition.class`, real definitions unmatched); `#define` macros had zero tags coverage in both; C typedef'd function pointers lost their alias name; C++ destructors and operator overloads completely untagged; C++ namespaces (plain + nested) had zero tags coverage; C++ explicit template specializations and out-of-line template-class methods lost their container tag; C++ template-argument calls (`identity<int>(5)`, `std::get<0>(x)`, etc. — the C++ analogue of Rust's turbofish gap) entirely unmatched; C++ `using` declarations/directives/aliases had zero imports coverage. Documented two non-bugs: `Type::~Type() = default;` at namespace scope is a genuine arborium-cpp grammar ambiguity; C preprocessor conditionals are deliberately excluded from complexity.
  - **Ruby**: `ruby.complexity.scm` never counted statement-modifier forms (`stmt if/unless/while/until cond`), inline `rescue` modifiers, independent `elsif` branches (only the outer `if` counted before), or Ruby 2.7+ `case ... in ...` pattern matching; bare Kernel-style calls with a constant callee (`Integer(x)`) unmatched in tags/calls; the receiver-qualified call pattern double-counted every receiver-qualified call (re-captured `@call` on top of the always-firing receiver-less pattern); `load`, `using` (refinement activation), and namespaced `include`/`extend`/`prepend` arguments unmatched in imports; dynamic/computed superclasses (`class Foo < Struct.new(...)`) unmatched in types. `class << self` singleton-class reopening investigated and left deliberately uncaptured (no honest name field exists), documented in `ruby.tags.scm`.

  **Incidental fix during merge**: two of the six parallel agents (Go, Ruby) independently discovered and fixed the same pre-existing bug — `tests/fixtures/cmake/CMakeLists.txt` was silently dropped by `.gitignore`'s blanket `*.txt` rule (only `expected/*.txt` was exempted), breaking `query_fixtures` compilation; both widened the exception and re-added the fixture. Deduplicated during the merge; kept the version already scoped to `tests/fixtures/**/*.txt`.

  **Confirmed pre-existing, unrelated, left untouched** (verified via `git stash`/independent runs by multiple sub-agents, out of scope for this sweep): `kdl::tests::unused_node_kinds_audit` (grammar node-kind drift in `crates/normalize-languages/src/kdl.rs`) and `normalize-facts`'s `extract_fixtures::ada/add_numbers` (fixture/output mismatch) — a separate concurrent session is already working the Ada fix.

  **✅ Batch 2 DONE (2026-07-28): C#, PHP, Swift, Kotlin, Scala, Haskell, Elixir, SQL** — all 8 applied via parallel worktree-isolated agents, each merged back with conflicts resolved by hand (CHANGELOG.md entry ordering only — every conflict this batch was purely mechanical, same as batch 1). Real, previously-silent bugs found and fixed per language (full detail in `CHANGELOG.md` `[Unreleased] Fixed`):
  - **C#**: `c-sharp.types.scm`'s `@type.reference` matched *every* identifier in the file (method/parameter/local names too), not just type positions — this grammar reuses plain `identifier` for both type and value slots, unlike Java/Rust; fixed with field-constrained patterns across variable/parameter/return/property/foreach/catch/cast/`is`/`as`/object-creation positions. `base_list` (extends+implements, undifferentiated in this grammar) entirely unhandled in tags; no `@definition.type` at all; generic/qualified/null-conditional invocation targets and `base(...)`/`this()` constructor delegation unmatched in calls+tags; aliased `using` produced duplicate/wrong `@import.path`; `switch_expression_arm` (C# 8+ expression-form switch) uncounted in complexity.
  - **PHP**: `php.tags.scm` had zero `@reference.*` captures at all (calls, `new`, extends, implements all invisible); `require`/`require_once` entirely unmatched in imports (distinct node from `include`); bare `use ClassName;` and grouped `use App\{Foo, Bar};` unmatched; trait composition (`use TraitA, TraitB;`) unmatched; aliased imports and union-type members double-counted; namespaced/dynamic/computed calls (`\Ns\func()`, `Class::$method()`, `$obj->$method()`, IIFEs) unmatched in calls; PHP 8 `match` arms and short-circuit operators uncounted in complexity.
  - **Swift**: every `extension` declaration invisible to tags (`extension`'s target is wrapped in `user_type`, not bare `type_identifier`, so no method/property inside an extension was ever nested) — same bug class as batch 1's Rust generic-impl gap; operator-overload function declarations entirely dropped; member `var`/`let` never captured; enum cases/protocol requirements/associatedtypes never captured; force-unwrap and generic-instantiation calls (`Array<Int>()`, parses as `constructor_expression` not `call_expression`) unmatched; `switch`/`guard`/`&&`/`||` undercounted or absent in complexity. Documented (not fixed): `init`/`deinit`/`subscript` have no populated `name` field in real parse output despite `node-types.json` claiming one — capturing them would need a `Language::node_name` override, out of scope for a query-only sweep.
  - **Kotlin**: paren-less interface implementation (`class Person(...) : Greeter`, the most common form) entirely unmatched; `by`-delegation unmatched; argument-carrying annotations misclassified as `@reference.class`; secondary-constructor delegation (`this(...)`/`super(...)`) unmatched in both calls+tags; aliased/wildcard imports double-counted, and — once fixed with an anchor — a trailing same-line comment after a plain import started silently dropping the import entirely (tree-sitter's documented anchor-skips-`extra`-nodes behavior does not hold for this grammar's comment attachment); `(user_type (type_identifier))` was a strict subset of its own blanket type pattern, double-counting every qualified/generic type; no `@definition.type` at all.
  - **Scala**: Scala 3 `enum` definitions entirely unmatched (a headline Scala 3 feature); operator-method definitions (`def +(other: Point)`) unmatched; zero `@reference.*` captures at all in tags; `with`-mixin traits invisible to any supertype query (`extends_clause.type` is declared `multiple: true` in node-types.json but only the *first* type after `extends` actually carries the field in practice — a live instance of the "field declared but not populated" trap); explicit operator-method calls and parenthesized call targets unmatched in calls; qualified type references double-counted; `Scala::extract_imports` left raw rename-arrow text (`Success => S`) in parsed names instead of clean names.
  - **Haskell**: cyclomatic complexity inflated for *every* function, unconditionally — `(match) @complexity` miscounted every equation-body wrapper (not just real branches) as a decision point; `multi_way_if`/`lambda_case`/`lambda_cases` contributed zero complexity; point-free/zero-argument bindings (`main = do ...`) entirely absent from tags (use a `bind` node, not `function`); `where`-bound local helpers leaked into top-level symbols; multiple `instance` declarations of the same typeclass for different types collapsed into one symbol (a Rust-side dedup-pass bug, not a query bug); custom-operator definitions (`(+++) xs ys = ...`) unmatched across all six declaration kinds; operator-section-as-prefix calls (`($) f x`) unmatched in calls; named imports never extracted at all.
  - **Elixir**: `elixir.complexity.scm` massively overcounted every function — blanket `(call)`+`(binary_operator)` patterns treated every function call and arithmetic operator as a decision point, since Elixir represents control flow as macro calls; guarded function/macro heads (`def name(x) when guard do`) entirely unmatched in tags; anonymous-function invocation (`fun.(args)`) unmatched in calls; multi-alias form (`alias Foo.{Bar, Baz}`) unmatched in imports.
  - **SQL**: `(when_clause) @complexity` never matched a single CASE...WHEN branch (that node is exclusive to MERGE's WHEN MATCHED/NOT MATCHED); `EXTRACT(YEAR FROM x)` produced two spurious calls; `CREATE TRIGGER`/`INDEX`/`SEQUENCE`/`MATERIALIZED VIEW` entirely unmatched in tags; zero `@reference.call` at all; function/procedure parameter types and `DECLARE`d variable types entirely unmatched in types; `CAST(expr AS type)` unmatched. Documented (grammar genuinely can't parse them): named `CONSTRAINT fk_name FOREIGN KEY` and PL/pgSQL `IF...THEN...ELSE...END IF` both produce ERROR nodes in this grammar version.

  **Incidental fix during merge**: `php`'s and `swift`'s `unused_node_kinds_audit` self-checks (`crates/normalize-languages/src/{php,swift}.rs`) each went stale after merging — every sub-agent ran its own audit test in an isolated worktree before the *other* language's changes existed, so neither's `documented_unused` list accounted for node kinds a sibling batch member had nothing to do with (a false "cross-contamination" red herring — actually just each list being correct only for its own single-language diff, never re-validated after all 8 branches were combined). Fixed by removing the now-stale entries from each list post-merge; verified `cargo test -p normalize-languages --lib` clean except the pre-existing `kdl` audit failure below.

  **Confirmed pre-existing, unrelated, left untouched**: `kdl::tests::unused_node_kinds_audit` (a separate concurrent agent is fixing it, per this task's instructions) and `normalize-facts`'s `extract_fixtures::c/add_numbers` (MATH_H header-guard misclassification — confirmed via `git diff --name-only` that no batch-2 sub-agent touched any C-related file). **✅ Fixed 2026-07-28, see "C/C++ header guard misclassification fix" section below.**

  **Remaining for future batches (73 of 99 registered grammars)**: ada, agda, asciidoc, asm, awk, batch, caddy, capnp, clojure, cmake, commonlisp, d, dart, devicetree, diff, dot, elisp, elm, erlang, fish, fsharp, gleam, glsl, graphql, groovy, hcl, hlsl, idris, ini, jinja2, jq, jsdoc, julia, kdl, lean, matlab, meson, nginx, ninja, nix, objc, ocaml, perl, postscript, powershell, prolog, query, r, rescript, ron, scheme, scss, sparql, ssh-config, starlark, svelte, textproto, thrift, tlaplus, tsx, typst, uiua, vb, verilog, vhdl, vim, vue, wit, x86asm, xml, yuri, zig, zsh. (Done so far: rust [prototype], python, typescript, javascript, go, java, c, cpp, ruby [batch 1]; c-sharp, php, swift, kotlin, scala, haskell, elixir, sql [batch 2]; json, yaml, toml, html, css, bash, lua, markdown, dockerfile [batch 3].)

  **✅ Batch 3 DONE (2026-08-07): JSON, YAML, TOML, HTML, CSS, Bash, Lua, Markdown, Dockerfile** — all 9 applied via parallel sub-agents sharing this checkout (no worktree isolation, per explicit instruction — see the process note above for the `GIT_INDEX_FILE`+`$$` hazard this surfaced and how it was caught/fixed). Real, previously-silent bugs found and fixed per language (full detail in `CHANGELOG.md` `[Unreleased] Fixed`):
  - **JSON**: empty-string keys (`"": value`) parse as a `string` node with no `string_content` child (grammar only emits that child for non-empty runs) — silently dropped from symbol extraction entirely; fixed at both the query and `node_name` level.
  - **YAML**: flow-mapping keys (`{key: value}`, the compact inline form) were entirely unextracted — only block-mapping keys were matched (see `CHANGELOG.md` for full detail on the anchor/alias/multi-document-stream completeness work alongside it).
  - **TOML**: arborium-toml's grammar has *no named fields at all* (`table`/`pair`/`table_array_element` all have empty `fields` objects) — the header/pair key is one of three sibling node kinds (`bare_key`/`quoted_key`/`dotted_key`); only `bare_key` was matched, so every dotted table header (`[tool.poetry]`, 40+ in this repo's own `Cargo.toml` files) and every dotted pair key (`a.b.c = 1`, 370+ in this repo) got no name at all — a severe, high-density gap.
  - **HTML**: unquoted attribute values (`<script src=app.js>`, legal HTML5, a distinct `attribute_value` node vs `quoted_attribute_value`) entirely unmatched in `html.imports.scm`; `<img src>` had no pattern at all; tag/attribute matching was case-sensitive via `#eq?` despite HTML being case-insensitive by spec.
  - **CSS**: arborium-css declares no `fields` on any node type; `@font-face`, `@layer`, `@property`, `@container`, and `@page` all parse as the same generic `at_rule` node and were entirely uncaptured (no symbols at all for any of them); `@scope` has its own dedicated node type, also uncaptured; `build_signature` silently truncated multi-token declaration values to their first token (`margin: 0 auto` rendered as `margin: 0`).
  - **Bash**: `bash.imports.scm` captured every trailing argument of `source file.sh arg1 arg2` as a spurious `@import.path` (bash passes trailing words as positional parameters, and the query's `argument:` field matched once per argument, `multiple`); `bash.complexity.scm` had no handling for `c_style_for_statement` (`for (( i = 0; i < n; i++ ))`, structurally distinct from the word-list `for_statement` — 4 real loops in this repo's own scripts affected), and was missing `ternary_expression` and the `&&`/`||` short-circuit forms of the overloaded `binary_expression` node.
  - **Lua**: `lua.calls.scm` only handled 3 of 6 `function_call.name` variants (bracket-index dispatch-table calls, parenthesized/IIFE calls, and chained/curried calls were all unmatched); bare-name function-expression assignment (`local f = function() end`) unhandled in `lua.tags.scm`; `lua.tags.scm` had no `@reference.call` patterns at all despite `lua.calls.scm` covering call sites.
  - **Markdown**: setext-style headings (`Heading\n===`/`Heading\n---`) entirely unmatched by `markdown.tags.scm` (only `atx_heading` was handled) — a real README/LICENSE-title convention with zero symbol representation before the fix. Confirmed no split inline sub-grammar exists in this arborium-markdown version (links/images/emphasis are opaque byte spans within `inline` nodes — a genuine capability boundary, documented not fabricated).
  - **Dockerfile**: `extract_imports`'s stage-alias extraction was dead code (`extract_stage_name` checked for a nonexistent `as_instruction` child kind — `as` is a direct field on `from_instruction`, so the condition could never be true, silently dropping every `FROM ... AS name` alias at the Rust trait level); `ARG`/`ENV` patterns in `dockerfile.tags.scm` weren't field-anchored, producing spurious symbols for default values (`ARG VERSION=1.0` tagged both `VERSION` and `1.0`); the two `FROM` patterns in `dockerfile.imports.scm` double-matched every aliased `FROM` (no `as:`-absence guard); added `COPY --from=<stage>` extraction for multi-stage builds.

  Next batch should continue with the config/markup and scripting tiers TODO.md previously flagged (e.g. json/yaml/toml siblings like ini/xml/scss, or the remaining scripting-adjacent set: awk, perl, powershell already covered in the CFG-only pass — same caveat as bash/lua/dockerfile applies, full methodology not yet run on their tags/calls/imports/complexity queries).
...
## C/C++ header guard misclassification fix (2026-07-28)

- [x] **Fixed the (then) only known failing test in the workspace**: `normalize-facts::extract_fixtures` on `c/add_numbers` — a classic header guard (`#ifndef MATH_H` / `#define MATH_H` / `...` / `#endif`) had its valueless `#define MATH_H` reported as a spurious `Function`-kind symbol (macros have no dedicated `SymbolKind`, so `definition.macro` maps to `Function`), a direct side effect of `30307f7a`'s earlier addition of macro-tags coverage to `c.tags.scm`/`cpp.tags.scm`. Root-caused via `normalize syntax ast`/`normalize syntax query`: the guard's `#define` is a `preproc_def` node with **no `value` field**, nested as the first child of a `preproc_ifdef` whose `name:` condition identifier textually matches the define's own name — a genuine structural pattern (`#eq?` + `!value` in the query), not a naming-convention hack (an `#ifndef DEBUG` / `#define DEBUG 1` default-value idiom, which shares its guard's name but *does* carry a value, is correctly left untouched — verified with a dedicated `/tmp` test file before deciding this was safe).
  - Fixed in the query, per CLAUDE.md's "node classification belongs in `.scm`" rule: added a `preproc_ifdef` pattern to both `c.tags.scm` and `cpp.tags.scm` (cpp has the identical bug — same shared C-preprocessor grammar shape, confirmed via `normalize view` before the fix showed `#define MATH_UTILS_HPP` in `cpp/add_numbers`'s fixture too, just never surfaced because the test panics on the first mismatch and `c` sorts before `cpp`) that tags the guard's `preproc_def` node with a new capture, `@_suppress`.
  - `@_suppress` is a **generic, language-agnostic convention**, not per-language Rust logic: `collect_symbols_from_tags` in `crates/normalize-facts/src/extract.rs` now does a first pass over all query matches collecting the byte-ranges of every `@_suppress`-captured node, then drops any `TagDef` whose node falls in that set — regardless of which pattern (in which language's query) produced the `@definition.*` capture for it. This was necessary rather than a single self-contained pattern because tree-sitter patterns are independent: a node matching two separate top-level patterns (the general macro pattern and the guard pattern) produces two separate matches, so suppression can't be expressed as one pattern alone.
  - Added `crates/normalize-languages/src/{c,cpp}.rs`'s `unused_node_kinds_audit` fix: `preproc_ifdef` moved out of the "documented unused" list since it's now genuinely used by the new guard pattern.
  - Added two permanent regression tests, `test_c_header_guard_suppressed`/`test_cpp_header_guard_suppressed` in `crates/normalize-facts/src/extract.rs`, covering both the suppression and the "shares-name-but-has-a-value" non-suppression case.
  - **Cascading discovery**: fixing the fail-fast `c/add_numbers` failure unmasked three more genuinely pre-existing, unrelated stale `normalize-facts` fixtures that had never been reached by the test before (same "fail-fast hides everything after the first mismatch" pattern as the earlier ada/SUMMARY.md and KDL fixes) — all confirmed as **the fixture being stale, not a new extraction regression**, before regenerating via `UPDATE_FIXTURES=1` and reviewing every diff:
    - `cpp/add_numbers`: C++ namespace nesting (`namespace math { ... }` wrapping its contents as a `Module` container) wasn't reflected — `30307f7a` added `namespace_definition` tags coverage earlier the same day but only regenerated `normalize-languages`' own smaller fixtures, not this end-to-end suite.
    - `haskell/add_numbers`: named imports (`import MathUtils (add, multiply)`) resolved to `line: 0`/`module: null` — the `import_name` capture patterns in `haskell.imports.scm` were standalone top-level patterns with no `@import` capture in the same match, so `collect_imports_with_compiled_query`'s `stmt_line` had nothing to read and silently defaulted to 0. **Fixed the root cause** (not blessed as fact in the fixture): restructured all five `import_name` variant patterns to nest inside the full `(import module: (module (module_id) @import.path) (import_list (import_name ...)))  @import` shape, verified via `normalize syntax query` to report the correct line and module for both.
    - `gleam/add_numbers` and `lua/add_numbers`: both fixtures were `[]` (empty) despite real extractable symbols existing in the fixture source — stale from earlier grammar/query restoration batches (`e7379be6`..`695b0159` CFG-extraction-restoration series, `2b29cc94` lua fix) that fixed the underlying `.scm`/grammar issue but never touched this fixture.
  - Verified every fixture diff individually before accepting (`git diff` per file) — all are additive (more/correct extraction, not less), none reflect a genuine extraction regression.
  - `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test -q`, and `NORMALIZE_REQUIRE_GRAMMARS=1 cargo test -q --workspace` all clean. **The workspace is fully green.**

---

## KDL `unused_node_kinds_audit` fix (2026-07-28)

- [x] **Fixed the last known failing test in the workspace** (`normalize-languages::kdl::tests::unused_node_kinds_audit`, tracked as pre-existing since before `d3618a5b`/`baf3a400`). Three flagged items, three different root causes — none were audit false positives (unlike the Lua case in `2b29cc94`, which *was* a formatting-only false positive):
  - `identifier_string` / `multi_line_string_body` in the `documented_unused` list **don't exist in the current `arborium-kdl` grammar** (verified via `dump_node_kinds` — the live node-kind list has no such kinds). Grammar version drift left two stale entries in `kdl.rs`'s `documented_unused` array; removed them.
  - `identifier` was a **genuine coverage gap**: KDL had zero symbol extraction (`crates/normalize-languages/src/queries/kdl.tags.scm` didn't exist, and `kdl` wasn't even registered in `bundled_tags_query`'s match in `grammar_loader.rs`). Added `kdl.tags.scm` — every `node`'s `identifier` (bare or quoted-string) is captured as `@definition.class` when it has a `children:` field (a `{ ... }` block, container — like a TOML table) or `@definition.var` otherwise (leaf entry, like a TOML pair), using tree-sitter's `!children` negated-field syntax to distinguish the two without any Rust-side branching. Registered `"kdl" => include_str!("queries/kdl.tags.scm")` in `bundled_tags_query`. Verified end-to-end with `normalize view` on a hand-written KDL fixture — nested symbols now show up correctly.
  - **Bonus fix, same file, same root cause (grammar drift)**: `Kdl::container_body` walked for a child of kind `node_no_terminator` before reading its `children` field — that wrapper node doesn't exist in the current grammar (confirmed via `normalize syntax ast`), so `container_body` always returned `None`, silently breaking KDL support in the edit service's "append/prepend to container" flows (`normalize-edit`'s `find_container_body`, used by `crates/normalize/src/service/edit.rs`). Fixed to read `node.child_by_field_name("children")` directly.
  - All grammar-structure claims verified against real parses via `normalize syntax ast`/`normalize syntax query` on hand-written `.kdl` fixtures (not read from grammar source), per CLAUDE.md's "use our own tools" rule.
  - No CHANGELOG entry: the `identifier_string`/`multi_line_string_body` and `container_body` fixes are dormant-bug-in-newly-exercised-code-path fixes with no prior working behavior to regress, and the `kdl.tags.scm` addition is new capability rather than a fix to an existing documented behavior — but given it turns "KDL has zero symbol extraction" into working `view`/edit support, this is arguably user-facing; flagged here for whoever next touches KDL to consider a CHANGELOG entry when KDL support is otherwise announced/documented.
  - **Newly discovered (not part of this fix, out of scope): four additional test failures are pre-existing in this environment whenever grammars are actually linked in** (i.e. surfaced by `cargo test --workspace`, not the default-member-only `cargo test`): `normalize-cfg`'s `go_cfg` tests (`QueryCompile("Query error at 27:4. Invalid node type \"expression_case_clause\"")`), `normalize-cfg`'s `java_cfg` snapshot tests (stale snapshots), `normalize-facts`' `extract_fixtures` (ada), and `normalize-scope::tests::test_rust_has_locals`. Confirmed via `git stash` against a clean pre-change tree — none are caused by this session's changes. This matches the shape of the older (no-longer-present-in-TODO.md) note about `java_cfg`/`lua_cfg`/`extract_fixtures` failing "whenever grammars are actually built" — the specific failing set has drifted (go_cfg instead of/in addition to lua_cfg-class issues, plus a new `normalize-scope` failure) since arborium grammar versions or the local grammar cache have moved on. Needs its own investigation session; not touched here since it's unrelated to the KDL audit and the task scope didn't include it.
...
**Done (2026-07-28, full subsumption):** `crates/normalize-vcs` scaffolded, then grown the same day to fully subsume `normalize-git`'s in-workspace surface per user decision ("every crate that touches git today should go through the VCS trait"). `Vcs` now covers every real operation used by a workspace caller: `repo_exists`, `commit_count`, `blame`, `blame_range_commits` (line-range blame for `view history`'s `git log -L`-style symbol history), `resolve_ref`, `resolve_merge_base`, `run_in_worktree` (git-specific, see below), `show`, `diff_name_status`, `diff_file_contents` (content-eager diff, replaces the `open_repo`+`diff_base_to_head`+`read_blob_text/bytes` pattern), `ls_files`, `remote_origin_url`, `commit_timestamps`, `log_timestamps`, `author_commit_counts`, `walk_commit_history` (combined per-commit file+churn+author walk — consolidates what were three separate `normalize-git` walks: `git_per_commit_files`, `git_file_churn_stats`, `git_activity_commits`, plus `normalize-facts`'s bespoke since-commit-aware co-change walk), `read_files_at_ref` (combined tree-walk + blob-read, replaces `open_repo`+`walk_tree_at_ref`+`read_blob_text`), `commits_since_last_touch` (staleness), and `recent_commit_messages` (commit-message embedding). Domain types added: `ChangeKind`, `FileContentChange`, `CommitEntry`, `AuthorCommitCount`, `CommitFileChange`, `CommitWalkEntry`, `CommitMessage`, `BlameRangeCommit` — no `gix` types in the public API.

**Worktree hard case, resolved:** `run_in_worktree` stays on the `Vcs` trait as a named git-specific capability, documented in the crate's doc comment as *not* backend-agnostic (`gix` has no worktree add/remove; jj's working-copy model has no equivalent "detached scratch checkout" primitive). No speculative fallback was designed for a hypothetical second backend — that's explicitly deferred to whoever adds one.

**Migrated (every in-workspace consumer, no half measures):** `normalize-git-history` (all of `activity.rs`, `contributors.rs`, `coupling.rs`, `repo_coupling.rs`, `ownership.rs`, `hotspots.rs`, `service.rs`), `normalize-ratchet` (`git_ops.rs`, `service.rs`), `normalize-budget` (`git_ops.rs` + all 6 metric files), `normalize-facts` (`index.rs`'s co-change walk — replaced its bespoke `gix` walk with `Vcs::walk_commit_history`), `normalize-semantic` (`git_staleness.rs` fully rewritten against `Vcs::commits_since_last_touch`; `populate.rs`'s commit-message loader — a `gix`-direct caller not caught by the initial `normalize_git::` grep — migrated to `Vcs::recent_commit_messages`), `normalize-architecture` (`service.rs`'s two diff-baseline call sites), and the main `normalize` crate (`commands/analyze/git_utils.rs` rewritten as a thin `Vcs` adapter preserving all existing call-site names; `commands/analyze/provenance.rs`'s `blame_file` now uses `Vcs::blame`; `commands/view/history.rs`'s line-range blame — another direct `gix` caller found only by a workspace-wide `gix::` grep, not the `normalize_git::` one — migrated to the new `Vcs::blame_range_commits`). `normalize-git`, `gix`, and (found unused as a drive-by cleanup) `git2` dropped as direct dependencies everywhere except `normalize-vcs` itself.

**Deprecation:** `normalize-git`'s `Cargo.toml` `description` and lib.rs doc comment now say "deprecated for in-workspace use, see normalize-vcs." Its public API is left otherwise intact — it's published on crates.io and may have external consumers depending on it directly as a low-level gix wrapper; unilaterally gutting its API wasn't done. Six functions (`git_head`, `git_head_branch`, `git_has_uncommitted_content_changes`, `git_summary_has_uncommitted_changes`, `git_last_commit_for_path`, `git_commit_count_for_path`, `resolve_ref_shellout`) were found to have zero callers anywhere in the workspace (verified via grep before this migration) — left in place rather than removed, since removing public API from a published crate is a breaking change that wasn't this task's call to make unilaterally.
...
**Implemented 2026-07-28.** `normalize sessions blame` lives in `normalize-sessions/src/blame.rs` (service method in `service.rs`), depending directly on `normalize-vcs` (not a new crate — extends the existing `sessions` verb, per the crate-existence test: no second dependent, and it's a natural extension of the crate that already owns session-log parsing/cross-referencing). Prior art found during implementation: `normalize view blame` (`crates/normalize/src/commands/analyze/provenance.rs`) already exists and does a *different*, weaker thing — it maps commit → session only when the session literally ran `git commit` in a Bash tool call whose output it can regex-match for the hash, at whole-file/commit granularity. It does not do content matching and can't attribute per-line within a commit that bundles several edits. `sessions blame` is the content-matching, per-chunk analog this TODO item asked for; the two commands coexist (different name, different algorithm, different confidence guarantees) — not a duplicate per CLAUDE.md's "check if it already exists" rule, since the mechanism and reliability differ, not just the wiring.

**Algorithm actually landed (see `blame.rs` module docs for full detail):**
1. `Vcs::blame` at HEAD gives, per current line, the commit that introduced content still there unchanged (standard blame semantics — no history-walk/remapping needed for this hop).
2. Consecutive current lines blamed to the same commit are grouped into a *chunk* — the attribution unit, deliberately smaller than a whole commit, since one commit routinely bundles multiple session edits (and possibly manual edits).
3. For the chunk's commit `C`, read file content at `C` and at `C^` (`Vcs::show`).
4. Candidate session edits = every `Edit`/`Write` tool call on this file (across scanned sessions) whose message timestamp is `<=` `C`'s commit timestamp. Timestamp order is used only as a hard validity filter (an edit can't postdate the commit it fed), never as a proximity/ranking heuristic.
5. A candidate matches when, after whitespace-only normalization (trailing space / CRLF, not fuzzy): the chunk's text is a substring of the edit's `new_string`; the edit's `old_string` (if any) is a substring of `C^`'s content; and the edit's `new_string` is a substring of `C`'s content. Zero matches → unattributed; more than one → ambiguous (listed, not guessed between) — this is what correctly resolves the "edit reverted/superseded within the same session" case (only the edit whose `new_string` equals what actually landed in the commit can match; a superseded earlier edit's stale `new_string` simply won't appear in `C`'s content).
6. A chunk whose normalized text has fewer than 6 non-whitespace characters is refused as unattributed ("too generic") even if exactly one candidate textually matches — short/generic content (a lone `}`, a blank line) is not reliable evidence.

**Known limitations, written down rather than silently accepted:** (a) a human touch-up after the last matching session edit but before commit is a false negative — reported unattributed, not misattributed, which is the intended reliability-over-recall trade; (b) path matching reuses `heatmap.rs`'s existing `normalize_path` heuristic (strips to a `src|lib|crates|tests|docs|packages` marker), so repos not using one of those directory names may under-match — an existing, already-accepted limitation elsewhere in this crate, not a new one; (c) same-commit renames aren't tracked (parent-content lookup uses the same path for `C^` as `C`), so a rename-in-the-same-commit can show as if newly created, turning a real match into a false-negative unattributed chunk — not attempted in this version; (d) edits with unknown/unparseable message timestamps are excluded from matching entirely (reliability over recall, matching the "no guessing" mandate).

Tests: `crates/normalize-sessions/src/blame.rs` has an end-to-end fixture (real `git init` + 3 commits + fabricated Claude-Code-format session JSONL) exercising all three outcomes (matched / ambiguous / unattributed) against known ground truth, plus small unit tests for the whitespace-normalization and meaningful-character-count helpers. `assert_output_formatter::<SessionsBlameReport>()` added to `crates/normalize/tests/output_formatter_impls.rs`. `cargo clippy --all-targets --all-features -- -D warnings && cargo test -q` green across the workspace.
