...
fn build_grammars(args: &[String]) {
    ...
    for (lang, crate_dir, version) in &grammars {
        ...
        match compile_grammar(lang, crate_dir, &out_dir, target, cc, version.as_deref()) {
            Ok(size) => {
...
/// Exported symbol name that carries the grammar's source version, baked into
/// the `.so` at compile time so it travels with the artifact (see
/// `grammar_version_source`). Read at runtime by
/// `normalize_languages::GrammarLoader` and folded into the content-addressed
/// cache key — a `.so` missing this symbol (hand-placed third-party grammar,
/// or a pre-injection release build) is treated as always-stale.
const GRAMMAR_VERSION_SYMBOL: &str = "normalize_grammar_version";

/// FNV-1a 64-bit hash, dependency-free. Used only to derive a stable version
/// tag for *local* grammars (`grammars/<lang>/`) which have no crate version
/// to embed — a content hash of the parser/scanner sources changes exactly
/// when the compiled `.so`'s behavior could change.
...
/// Write a small generated C translation unit exporting `normalize_grammar_version`
/// as a `const char[]` array (NOT a `const char *` pointer variable) holding
/// `version`, so it becomes a real symbol in the resulting `.so`/`.dylib`/`.dll`.
///
/// Declared as an array deliberately: `dlsym`/`GrammarLoader::get` reads a data
/// symbol's address and reinterprets that address itself as the value (see
/// `normalize_languages::grammar_loader::read_grammar_version_symbol` — the same
/// single-indirection convention already used for the grammar's own
/// `tree_sitter_<lang>` function symbol). For an array, the symbol's address IS
/// the address of the string's first byte, so a single indirection reaches the
/// text directly. A `const char *` pointer *variable* would instead put the
/// symbol's address at the pointer slot itself, requiring a second dereference
/// to reach the string — a real bug caught by
/// `test_grammar_version_present_when_built_by_xtask` before this comment existed.
///
/// Written to `out_dir` alongside the grammar (deleted by the caller once
/// compilation finishes, successfully or not) so it never collides with a
/// concurrent build of a different grammar.
fn write_grammar_version_source(
    out_dir: &Path,
    lang: &str,
    version: &str,
) -> Result<PathBuf, String> {
    let path = out_dir.join(format!("{lang}.normalize-version.c"));
    // `version` is always either an arborium semver (digits/dots) or our own hex
    // hash, so it can never contain a `"` or `\` that would break out of the
    // C string literal — but guard anyway rather than trust the caller forever.
    if version.contains('"') || version.contains('\\') {
        return Err(format!(
            "grammar version string for '{lang}' contains unsupported characters: {version:?}"
        ));
    }
    let contents = format!(
        "/* Generated by `cargo xtask build-grammars` — do not edit. */\n\
         const char {GRAMMAR_VERSION_SYMBOL}[] = \"{version}\";\n"
    );
    fs::write(&path, contents).map_err(|e| format!("failed to write version source: {e}"))?;
    Ok(path)
}
...
fn compile_local_grammar(
    ...
    // Local grammars have no crate version, so derive a stable version tag from
    // the content of their sources instead.
    let mut hash_input =
        fs::read(&parser_c).map_err(|e| format!("failed to read {}: {e}", parser_c.display()))?;
    if scanner_c.exists() {
        hash_input.extend(
            fs::read(&scanner_c)
                .map_err(|e| format!("failed to read {}: {e}", scanner_c.display()))?,
        );
    }
    let version = format!("local-{:016x}", fnv1a64(&hash_input));
    let version_c = write_grammar_version_source(out_dir, lang, &version)?;
    ...
    let output = cmd.output().map_err(|e| {
        let _ = fs::remove_file(&version_c);
        format!("Failed to run {compiler_display}: {e}")
    })?;
    let _ = fs::remove_file(&version_c);
...
/// `(lang, crate_dir, arborium_version)`. `arborium_version` is `None` only if the
/// registry directory name didn't carry a recognizable semver suffix (shouldn't
/// happen for real arborium crates, but callers must not fabricate a version in
/// that case — see `grammar_version_source`).
fn find_arborium_grammars(registry_src: &Path) -> Vec<(String, PathBuf, Option<String>)> {
    let mut grammars = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        for crate_entry in crates.flatten() {
            ...
            if let Some(lang) = name.strip_prefix("arborium-") {
                ...
                // Split off the version suffix (e.g., "c-sharp-2.4.5" -> "c-sharp" / "2.4.5")
                // Version is always at the end in format X.Y.Z
                let split = split_version_suffix(lang);

                // Check grammar exists
                if crate_dir.join("grammar/src/parser.c").exists() {
                    let lang = split.base.to_string();
                    let version = split.version.map(str::to_string);
                    grammars.push((lang, crate_dir, version));
                }
            }
        }
    }
...
fn compile_grammar(
    ...
    // Embed the arborium crate version as an exported symbol so it travels with
    // the compiled artifact (cache-key purposes — see GRAMMAR_VERSION_SYMBOL).
    // A `.so` with no version symbol is deliberately possible: if for some reason
    // we couldn't determine the arborium version, we do NOT fabricate one — the
    // runtime treats a missing symbol as always-stale, which is the safe default.
    let version_c = match version {
        Some(v) => Some(write_grammar_version_source(out_dir, lang, v)?),
        None => {
            eprintln!(
                "  {lang}: WARNING - could not determine arborium crate version; \
                 built without a version symbol (cache will always treat it as stale)"
            );
            None
        }
    };
    ...
    if let Some(ref version_c) = version_c {
        cmd.arg(version_c);
    }
...
/// Result of splitting a crate name into its base and trailing version, see
/// [`split_version_suffix`].
struct VersionSplit<'a> {
    base: &'a str,
    version: Option<&'a str>,
}

/// Split a crate name into a base and version by matching a trailing semver-looking
/// segment (e.g., "c-sharp-2.4.5" -> base "c-sharp", version "2.4.5"). Cargo crate
/// names end with `-MAJOR.MINOR.PATCH`, so this is how we recover the arborium
/// grammar crate's version — which xtask needs to embed into the compiled `.so` for
/// cache-key purposes (see `write_grammar_version_source`).
fn split_version_suffix(name: &str) -> VersionSplit<'_> {
