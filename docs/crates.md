...
| `normalize-vcs` | VCS-agnostic trait boundary (`CommitId`, `BlameLine`, `Vcs` trait) with a `GitBackend` wrapping `normalize-git` | — | Added 2026-07-28. Domain-typed boundary extraction — not a green-field API — for the operations that actually crossed a `gix`-typed boundary in current consumers (blame, repo-exists, commit-count). Git is the only backend; jj/Mercurial are future work (see `TODO.md` "VCS abstraction layer"). `normalize-git-history`'s blame/ownership path and two existence/count checks (hotspots, coupling-clusters) migrated to depend on it instead of raw `gix` types. |
