...
| `normalize-git` | **Deprecated for in-workspace use — depend on `normalize-vcs` instead.** Pure-Rust read-only git operations: repo open, blob read, tree walk, diff, blame, churn, history | — | Extracted 2026 (B1) to dedup gix helpers across budget/ratchet/semantic/native-rules/main. As of the 2026-07-28 "fully subsume normalize-git" migration, `normalize-vcs` is the crate's only in-workspace dependent — every other former direct consumer (`normalize-git-history`, `normalize-ratchet`, `normalize-budget`, `normalize-facts`, `normalize-semantic`, `normalize-architecture`, main `normalize`) now goes through `normalize_vcs::Vcs` instead. Public API left intact (still published, still useful standalone as a low-level gix wrapper for consumers outside this workspace) — nothing removed as a breaking change, but new in-workspace code should not add a direct dependency on it. |
...
| `normalize-vcs` | VCS-agnostic trait boundary (`CommitId`, `BlameLine`, `CommitWalkEntry`, `FileContentChange`, ... + `Vcs` trait) with a `GitBackend` wrapping `normalize-git` | — | Added 2026-07-28; grown the same day to fully subsume `normalize-git`'s in-workspace surface. `Vcs` now covers every real git operation used across the workspace: blame (whole-file and line-range), ref/merge-base resolution, worktree materialization (git-specific, documented as such — doesn't generalize to jj), file content at a ref, name-status and full-content diffs, tracked-file listing, remote URL, commit timestamps/log/author-counts, a combined per-commit file+churn+author walk, commit-message access, and staleness (commits-since-last-touch). Every former direct `normalize-git` consumer (`normalize-git-history`, `normalize-ratchet`, `normalize-budget`, `normalize-facts`, `normalize-semantic`, `normalize-architecture`, main `normalize`) now depends on this crate instead. Git is the only backend; jj/Mercurial are future work (see `TODO.md` "VCS abstraction layer"). |
