...
- **Known-broken-grammar visibility for Zsh.** `normalize_languages::known_broken_grammar`
  ...
  returns a clear error instead of a garbage CFG; `normalize syntax ast` warns before
  showing the (genuinely broken) tree. All five `zsh.*.scm` query files now carry a
  header documenting the same.
