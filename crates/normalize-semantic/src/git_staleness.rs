...
pub fn compute_staleness_batch(
    ...
    for rel_path in unique_paths {
        let staleness = compute_staleness_for_file(root, rel_path);
...
///
/// `staleness = min(1.0, commits_since_last_touch / 50.0)`, or `0.0` if no commit ever
/// touched the file (likely untracked).
fn compute_staleness_for_file(root: &Path, rel_path: &str) -> f64 {
    let Some(commits_before_last_touch) = GitBackend.commits_since_last_touch(root, rel_path)
    else {
        return 0.0;
    };
