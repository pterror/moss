...
; arborium-typescript's node-types.json), which expands through
; `primary_expression` to ~20 node kinds. The clauses below cover every variant
; confirmed to actually occur as a direct call target via `normalize syntax
; query`/`normalize syntax ast` on real TS source (see
; docs/query-testing-methodology.md). `instantiation_expression` (an explicit
; generic reference with no call, e.g. `const f = identity<number>;`) never
; appears directly as `call_expression.function` in practice — only nested
; inside a `parenthesized_expression`, which the parenthesized-call clause
; below already captures as a whole.
